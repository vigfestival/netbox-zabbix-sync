...
def main(arguments):
    ...
    #Go through all netbox virtual machines
    for nb_vm in netbox_vms:
        try:
            vm = NetworkVirtualmachine(nb_vm, zabbix, netbox_journals,
                                   arguments.journal)
            vm.set_hostgroup(arguments.layout)
            # Checks if device is part of cluster.
            # Requires the cluster argument.
            if(vm.isCluster() and arguments.cluster):
                # Check if device is master or slave
                if(vm.promoteMasterDevice()):
                    e = (f"Device {vm.name} is "
                         f"part of cluster and primary.")
                    logger.info(e)
                else:
                    # Device is secondary in cluster.
                    # Don't continue with this device.
                    e = (f"Device {vm.name} is part of cluster "
                         f"but not primary. Skipping this host...")
                    logger.info(e)
                    continue
            # Checks if device is in cleanup state
            if(vm.status in zabbix_device_removal):
                if(vm.zabbix_id):
                    ...
                    # and remove hostID from Netbox.
                    vm.cleanup()
                    logger.info(f"Cleaned up host {vm.name}.")

                else:
                    # Device has been added to Netbox
                    # but is not in Activate state
                    logger.info(f"Skipping host {vm.name} since its "
                                f"not in the active state.")
                continue
            elif(vm.status in zabbix_device_disable):
                vm.zabbix_state = 1
            # Add hostgroup is flag is true
            # and Hostgroup is not present in Zabbix
            if(arguments.hostgroups):
                for group in zabbix_groups:
                    # If hostgroup is already present in Zabbix
                    if(group["name"] == vm.hostgroup):
                        break
                else:
                    # Create new hostgroup
                    hostgroup = vm.createZabbixHostgroup()
                    zabbix_groups.append(hostgroup)
            # Device is already present in Zabbix
            if(vm.zabbix_id):
...
class NetworkVirtualmachine():

    """
    Represents Network virtualmachine.
    INPUT: (ZabbixAPI class, journal flag, NB journal class)
    """
    ...
    def _setBasics(self):
        ...
        Sets basic information like IP address.
        """
        # Return error if virtualmachine does not have primary IP.
        if(self.nb.primary_ip):
            self.cidr = self.nb.primary_ip.address
            self.ip = self.cidr.split("/")[0]
        else:
        ...
        # Check if device_type has custom field
        vm_cf = self.nb.custom_fields

        if(vm_template_cf in vm_cf):
            self.zabbix_vm_template = self.nb.custom_fields[vm_template_cf]
        else:
    ...
    def getZabbixTemplate(self, templates):
        ...
        for template in templates:
            if(template['name'] == self.vm_template_name):
                self.template_id = template['templateid']
                e = (f"Found template ID {str(template['templateid'])} "
                     f"for host {self.name}.")
                logger.debug(e)
